...
from .utils import sample_image, sample_video,sample_video_from_bytes, sample_gif, get_visual_type

ext2sampler = {
    'image': sample_image,
    'gif': sample_gif,
    'video': sample_video,
    'byte':sample_video_from_bytes
}
...
class Processor(object):
    ...
    def load_images(self, visual_data_path, n_frames=None, start_time=0, end_time=-1):
        ...
        return sample_video_from_bytes(visual_data_path, n_frames=min(n_frames, self.max_n_frames) if n_frames else self.max_n_frames, start_time=start_time, end_time=end_time)
